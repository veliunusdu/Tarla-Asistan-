...
public class RequestOtpCommandHandlerTests
{
    ...
    private IConfiguration CreateConfig(int cooldownSeconds = 60, int ttlSeconds = 180, string env = "Development")
    {
    ...
    [Fact]
    public async Task Handle_WhenValidRequest_ShouldCreateOtpAndReturnDebugOtpInDevelopmentEnv()
    {
        // Arrange
    ...
    [Fact]
    public async Task Handle_WhenProductionRequest_ShouldNotReturnDebugOtp()
    {
        // Arrange
        var db = new MockDbContextBuilder().Build();
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["Auth:OtpTtlSeconds"] = "180",
                ["Environment"] = "local",
                ["ASPNETCORE_ENVIRONMENT"] = "Production"
            })
            .Build();
        var handler = new RequestOtpCommandHandler(db, config, _loggerMock.Object);
        var command = new RequestOtpCommand("+905551234568");

        // Act
        var result = await handler.Handle(command, CancellationToken.None);

        // Assert
        result.DebugOtp.Should().BeNull();
    }
