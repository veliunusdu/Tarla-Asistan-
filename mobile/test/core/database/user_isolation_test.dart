...
void main() {
  ...
  group('Migrations.v4ToV5', () {
    test(
      'tarlalar, faaliyetler ve sync_operations tablolarına userId kolonu ekler ve verileri korur',
      () async {
        ...
        final tCols = (await db.rawQuery(
          'PRAGMA table_info(tarlalar)',
        )).map((r) => r['name'] as String).toSet();
        final fCols = (await db.rawQuery(
          'PRAGMA table_info(faaliyetler)',
        )).map((r) => r['name'] as String).toSet();
        final sCols = (await db.rawQuery(
  ...
  group('DatabaseHelper User Isolation', () {
    ...
    test('Kullanıcı A tarlaları kullanıcı B tarafından görünmez', () async {
      ...
      currentTestUser = 'userA';
      await dbHelper.insertTarla(
        const Tarla(id: 't-userA', name: 'User A Tarlası', size: 10.0),
      );
      ...
      await dbHelper.insertTarla(
        const Tarla(id: 't-userB', name: 'User B Tarlası', size: 20.0),
      );
    ...
    test(
      'Kullanıcı B kullanıcı A tarlasını güncelleyemez ve silemez',
      () async {
        currentTestUser = 'userA';
        await dbHelper.insertTarla(
          const Tarla(id: 't-userA', name: 'User A Tarlası', size: 10.0),
        ...
        currentTestUser = 'userB';
        // Attempt update
        final updateCount = await dbHelper.updateTarlaLocation(
          't-userA',
          38.0,
          35.0,
        );
    ...
    test(
      'Kullanıcı A faaliyetleri ve sync operasyonları kullanıcı B ile karışmaz',
      () async {
        currentTestUser = 'userA';
    ...
    test(
      'clearUserData oturumu kapatılan kullanıcının yerel verilerini temizler',
      () async {
        currentTestUser = 'userA';
        await dbHelper.insertTarla(
          const Tarla(id: 't-userA', name: 'User A Tarlası'),
        );
        await dbHelper.insertFaaliyetWithSync(
          Faaliyet(
            id: 'f-userA',
        ...
        currentTestUser = 'userB';
        await dbHelper.insertTarla(
          const Tarla(id: 't-userB', name: 'User B Tarlası'),
        );
    ...
    test(
      'clearUserData pending_task_actions tablosundaki kullanıcıya ait bekleyen aksiyonları temizler',
      () async {
    ...
    test(
      'clearUserData gönderilmemiş vaka fotoğrafını ve sesini temizler',
      () async {
        final db = await dbHelper.database;
        await db.execute('''
        CREATE TABLE pending_case_submissions (
          id TEXT PRIMARY KEY,
      ...
      ''');
        ...
        final image = File('${tempDir.path}${Platform.pathSeparator}photo.jpg');
        final audio = File('${tempDir.path}${Platform.pathSeparator}voice.m4a');
        ...
        await db.insert('pending_case_submissions', {
          ...
          'local_audio_path': audio.path,
        });

        await dbHelper.clearUserData(userId: 'userA');

        expect(await db.query('pending_case_submissions'), isEmpty);
    ...
    test(
      'Sahipsiz eski kayıtlar otomatik olarak yeni kullanıcıya SIZMAZ ve çalınamaz (Anti-Hijacking)',
      () async {
    ...
    test('clearOrphanedRecords sahipsiz kayıtları temizler', () async {
      currentTestUser = null;
      await dbHelper.insertTarla(
        const Tarla(id: 't-orphan-del', name: 'Silinecek Sahipsiz Tarla'),
      );
