...
void main() {
  ...
  group('forecast timestamp regression', () {
    Future<WeatherSummary> load(
      List<dynamic> points, {
      ...
      final client = makeClient(
        handler: (_) async => http.Response(
          jsonEncode({
            ...
            'current': current,
            'is_stale': stale,
            'stale_reason': stale ? 'Cached forecast' : null,
            'risks': [],
          }),
          200,
        ),
      ...
      return BackendWeatherRepository(
        apiClient: client,
        farmId: 'timestamp-farm',
        localRepo: FakeLocalWeatherRepository(),
      ).getWeather();
    }

    for (final stale in [true, false]) {
      test(
        'selects the closest hour instead of midnight, stale=$stale',
        () async {
          ...
          expect(summary.humidity, 40);
          expect(summary.windSpeed, 12);
          expect(summary.observedAt, now);
          expect(summary.isStale, stale);
          expect(summary.staleReason, stale ? 'Cached forecast' : null);
        },
      );
    }

    test(
      'ignores invalid timestamps and compares offset timestamps as instants',
      () async {
        final now = DateTime.now().toUtc();
        final plusThree =
            '${now.add(const Duration(hours: 3)).toIso8601String().replaceAll('Z', '')}+03:00';
        final summary = await load([
          {'observed_at': 'invalid', 'temperature_c': -90},
          {'temperature_c': -80},
          {'observed_at': plusThree, 'temperature_c': 26},
        ]);
        expect(summary.temperature, 26);
        expect(summary.observedAt, now);
      },
    );

    test('does not present an untimed forecast as current weather', () async {
      final summary = await load([
        {'temperature_c': 3, 'observed_at': 'invalid'},
      ]);
      expect(summary.temperature, isNull);
      expect(summary.observedAt, isNull);
    });

    test('keeps actual current weather ahead of forecast fallback', () async {
      final now = DateTime.now().toUtc();
      final summary = await load(
        [
          {'observed_at': now.toIso8601String(), 'temperature_c': 25},
        ],
        current: {'observed_at': now.toIso8601String(), 'temperature_c': 23},
      );
      expect(summary.temperature, 23);
    });
  });
  ...
  group('BackendWeatherRepository – canonical ApiClient', () {
        ...
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({
              ...
              'current': {'temperature_c': 20, 'condition': 'Parçalı Bulutlu'},
              'risks': [],
            }),
            200,
    ...
    test(
      'returns WeatherSummary with null temperature when points list and current are empty',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
    ...
    test(
      'maps a selected farm without coordinates to the location flow',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
    ...
    test(
      'maps a weather provider outage to a typed retryable outcome',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
    ...
    test(
      'returns cached weather with clean description, isStale=true, and staleReason when API returns 503 and cache exists',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            '{"detail":"Hava durumu şu anda alınamıyor."}',
    ...
    test(
      'returns cached weather with clean description, isStale=true, and staleReason when API throws network/timeout error and cache exists',
      () async {
        final failClient = ApiClient(
          httpClient: MockClient(
    ...
    test(
      'rethrows WeatherUnavailableException when API returns 503 and cache is empty',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            '{"detail":"Hava durumu şu anda alınamıyor."}',
      ...
      final client = makeClient(
        handler: (_) async => http.Response(
          jsonEncode({
            ...
            'current': {'temperature_c': 28.0, 'condition': 'Açık'},
            'points': [],
            'risks': [],
          }),
          200,
  ...
  group('BackendWeatherRepository – 11 domain mapping scenarios', () {
    test(
      '1. Full response preserves decimals and all fields without loss',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({
        ...
        expect(
          summary.risks.first.suggestedAction,
          'Sulama planını kontrol edin',
        );
        client.close();
      },
    );
    ...
    test('2. Real 0°C is preserved and not confused with null', () async {
      final client = makeClient(
        handler: (_) async => http.Response(
          jsonEncode({
            'current': {'temperature_c': 0.0, 'condition': 'Açık'},
            'risks': [],
          }),
          200,
          headers: {'content-type': 'application/json; charset=utf-8'},
    ...
    test(
      '3. Current is null: falls back safely to a timestamped forecast point',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({
    ...
    test(
      '4. Both current and points absent: no crash, returns WeatherSummary with null temperature',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({'current': null, 'points': [], 'risks': []}),
            200,
            headers: {'content-type': 'application/json; charset=utf-8'},
          ),
        );
    ...
    test(
      '5. Daily is null: no crash, min/max and rain probability are null',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({
              'current': {'temperature_c': 20.0, 'condition': 'Açık'},
              'daily': null,
              'risks': [],
            }),
            200,
    ...
    test('6. Daily is empty list: no crash, min/max are null', () async {
      final client = makeClient(
        handler: (_) async => http.Response(
          jsonEncode({
            'current': {'temperature_c': 22.0, 'condition': 'Açık'},
            'daily': [],
            'risks': [],
          }),
          200,
    ...
    test(
      '9. Stale data preserves weather condition and sets isStale and staleReason',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({
    ...
    test(
      '10. Numeric JSON variations: int, float, decimal safely parsed without precision loss',
      () async {
        ...
        final repoInt = BackendWeatherRepository(
          apiClient: clientInt,
          farmId: 'farm-num-int',
        );
        ...
        final repoFloat = BackendWeatherRepository(
          apiClient: clientFloat,
          farmId: 'farm-num-float',
        );
        ...
        final repoDecimal = BackendWeatherRepository(
          apiClient: clientDecimal,
          farmId: 'farm-num-dec',
        );
    ...
    test(
      '11. Malformed optional DateTime does not crash and leaves fields null',
      () async {
        final client = makeClient(
          handler: (_) async => http.Response(
            jsonEncode({
              'fetched_at': 'corrupt-time',
              'current': {'temperature_c': 19.0, 'observed_at': 'not-a-date'},
              'risks': [
                {
                  'risk_type': 'WIND',
                  'severity': 'MEDIUM',
