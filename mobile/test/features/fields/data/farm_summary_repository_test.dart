...
void main() {
  group('FarmSummaryResponse parsing', () {
    test(
      'parses full response with farms, nextTask, lastActivity, and upcomingTasks',
      () {
        final json = {
          'farms': [
            {
        ...
        expect(farmSummary.tarla.currentCropPeriodId, 'cp1');
    ...
    test('supports null nextTask and null lastActivity safely', () {
      ...
      expect(response.farms.first.tarla.currentCropPeriodId, isNull);
  ...
  group(
    'BackendTarlaRepository.getFarmSummary single HTTP request guarantee',
    () {
      test(
        'makes exactly 1 HTTP GET request even when 10 farms are returned',
        () async {
          int requestCount = 0;
          ...
          final client = ApiClient(
            httpClient: MockClient((request) async {
              ...
              return http.Response(
                jsonEncode({'farms': tenFarmsJson, 'upcoming_tasks': []}),
                200,
          ...
          expect(
            requestedPath,
            contains('/api/v1/farms/summary?upcomingLimit=5'),
          );
        },
      );
    },
