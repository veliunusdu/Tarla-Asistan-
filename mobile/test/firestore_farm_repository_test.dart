...
void main() {
  ...
  test(
    'writes activity under its farm with description and voice provenance',
    () async {
      final store = _RecordingStore();
      final repository = FirestoreFarmRepository(uid: 'uid-1', store: store);
      ...
      expect(
        store.data.keys,
        unorderedEquals(FirestoreFarmContract.activityFields),
      );
  ...
  test(
    'decodes Firestore timestamps and retains activity description and input method',
    () async {
      final store = _RecordingStore();
      final repository = FirestoreFarmRepository(uid: 'uid-1', store: store);
      store.activities = [
  ...
  test('rejects a farm document with an invalid required field type', () async {
    ...
    final repository = FirestoreFarmRepository(uid: 'uid-1', store: store);
    store.farms = [
      FirestoreFarmDocument(
        id: 'f1',
        data: {..._farmData(), 'latitude': '38.7'},
      ),
    ];

    await expectLater(
      repository.watchFarms('uid-1').first,
  ...
  test('declares the named database and no client deletion operations', () {
    ...
    expect(
      ...
      containsAll(['activityType', 'inputMethod']),
    );
  });

  test(
    'production adapter selects named database and executes owner query',
    () async {
  ...
  test(
    'production adapter writes encoded fields to the requested document',
    () async {
      final firestore = FakeFirebaseFirestore();
      final store = FirebaseFirestoreFarmStore(
        instanceFactory: _RecordingInstanceFactory(firestore),
      ...
      final writeObserved = expectLater(
        document.snapshots(),
        emitsThrough(
          predicate<DocumentSnapshot<Map<String, dynamic>>>(
...
Faaliyet _activity({String inputMethod = 'MANUAL'}) => Faaliyet(
  id: 'a1',
  tarlaId: 'f1',
  type: 'OTHER',
  note: 'Damlama sulama yapıldı.',
...
class _RecordingStore implements FirestoreFarmStore {
  ...
  @override
  Stream<FirestoreWriteFailure> get writeFailures => _failures.stream;
  void failLater(String path, Object error) =>
      _failures.add(FirestoreWriteFailure(path: path, cause: error));
  @override
  Future<void> setDocument(String value, Map<String, Object?> valueData) async {
    path = value;
    data = Map.of(valueData);
  }

  @override
  Stream<List<FirestoreFarmDocument>> watchFarms(String ownerId) {
    watchedOwnerId = ownerId;
    return Stream.value(farms);
  }
