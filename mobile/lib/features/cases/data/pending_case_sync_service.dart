...
class PendingCaseSyncService {
  ...
  Future<int> syncPending() async {
    ...
    for (var submission in pending) {
      try {
        if (submission.uploadedMediaId == null && submission.localImagePath != null) {
          final bytes = await File(submission.localImagePath!).readAsBytes();
          final response = await _api.postMultipart('/media', files: [
            ApiMultipartFile(field: 'file', bytes: bytes, filename: '${submission.id}.jpg', contentType: 'image/jpeg'),
          ]);
          final mediaId = response['id']?.toString();
          if (mediaId == null || mediaId.isEmpty) throw const ApiException('Fotoğraf kimliği alınamadı.');
          submission = submission.copyWith(uploadedMediaId: mediaId);
          await _repository.update(submission);
        }

        final mediaIds = submission.uploadedMediaId == null ? null : [submission.uploadedMediaId!];
        ...
        final updated = submission.copyWith(
          ...
          state: _isTerminal(error) ? PendingCaseState.failed : PendingCaseState.pending,
        );
        await _repository.update(updated);
        if (!_isTerminal(error) || error.statusCode == 401 || error.statusCode == 403) return synced;
