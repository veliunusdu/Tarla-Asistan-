...
class PendingCaseSubmission {
  ...
  factory PendingCaseSubmission.fromMap(Map<String, dynamic> map) => PendingCaseSubmission(
        id: map['id'] as String,
        ...
        lastAttemptAtUtc: map['last_attempt_at_utc'] == null ? null : DateTime.tryParse(map['last_attempt_at_utc'] as String)?.toUtc(),
        lastErrorCode: (map['last_error_code'] as num?)?.toInt(),
        state: PendingCaseState.values.firstWhere((s) => s.name == map['state'], orElse: () => PendingCaseState.pending),
      );
  ...
  PendingCaseSubmission copyWith({
    ...
    PendingCaseState? state,
        ...
        title: title, description: description, clientOperationId: clientOperationId,
        localImagePath: localImagePath, uploadedMediaId: uploadedMediaId,
        createdAtUtc: createdAtUtc, attemptCount: attemptCount ?? this.attemptCount,
